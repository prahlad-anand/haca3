#!/usr/bin/env python3
"""
Travel-subjects imputation evaluation for HACA3 (non-contrastive / theta-encoder variant).
...
def main(args=None):
    ...
    parser = argparse.ArgumentParser(
        description='HACA3 travel-subjects imputation evaluation (non-contrastive).'
    )
    ...
    device = haca3.device
    for m in [haca3.beta_encoder, haca3.eta_encoder, haca3.theta_encoder,
    ...
    with torch.no_grad():
        for batch_id, image_dicts in enumerate(tqdm(loader, desc='Batches')):
            ...
            thetas_all, _, _ = haca3.calculate_theta(imgs)  # list of [B, theta_dim, 1, 1]
