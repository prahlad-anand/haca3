...
def save_image(images, file_name):
    ...
    image_save = utils.make_grid(tensor=image_save, nrow=4, normalize=False, value_range=(0,1)).detach().numpy()[0, ...]
...
#         # images is a list; image is a tensor of shape [B,1,H,W]
#         # take only the first sample in batch
#         img = image[0:1, [0], ...].cpu()  # [1, 1, H, W]

#         # Make a row of 4 tiles (will repeat or need exact 4 channels)
#         grid = make_grid(img, nrow=4)

#         # Convert to HWC for saving
#         grid = grid.detach().cpu().numpy().transpose(1, 2, 0)

#         print("before", file_name, grid.min(), grid.max())
...
#         # ---- FIX END ----
#         if (grid_uint8.min()<0 or grid_uint8.max()>255):
#             print("ACHTUNG!\n\n\n\n\n\n\n", grid_uint8.min(), grid_uint8.max())
#         print("after", file_name, grid_uint8.min(), grid_uint8.max())

#         imageio.imwrite(f"{file_name}_{idx}.png", grid_uint8)
