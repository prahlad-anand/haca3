#!/usr/bin/env python3
"""
Imputation evaluation for HACA3.
...
def main(args=None):
    ...
    # ── contrast encoder (identical to evaluate.py) ───────────────────────
    ...
    device = haca3.device   # may have changed if CUDA not available
    ...
    with torch.no_grad():
        for batch_id, image_dicts in enumerate(tqdm(valid_loader, desc='Batches')):
            ...
            # list input → list of [B, contrast_dim]
            contrasts_all  = haca3.calculate_contrast(imgs)
            ...
            # keys for all 4 sources: list of [B, contrast_dim+eta_dim, 1, 1]
            keys_all = [
                torch.cat([
            ...
            for target_idx in range(4):
                ...
                # target contrast embedding (for decode query)
