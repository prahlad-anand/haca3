...
class HACA3Evaluator:
    ...
    def evaluate_batch(self, image_dicts):
        ...
        contrast_target = self.haca3.calculate_contrast(target_image).unsqueeze(2).unsqueeze(3) # .repeat(1,3,1,1)
        ...
        contrast_rec = self.haca3.calculate_contrast(rec_image).unsqueeze(2).unsqueeze(3) # .repeat(1, 3, 1, 1)
...
def save_images_from_dicts(image_dicts, save_dir, prefix="sample"):
    os.makedirs(save_dir, exist_ok=True)

    for i, d in enumerate(image_dicts):
        img = d["image"]  # shape [1, 1, H, W] or [1, C, H, W]
        ...
        img_min = img.min()
        img_max = img.max()
        print("img_min_max at dict:", img_min, img_max)
        print(h)
        if img_max > img_min:
            img = (img - img_min) / (img_max - img_min)
        else:
        ...
        save_path = os.path.join(save_dir, f"{prefix}_contrast{i}.png")
        vutils.save_image(img, save_path)
...
def main(args=None):
    ...
    if args.pretrained_contrast_encoder is not None:
        checkpoint_contrast_encoder = torch.load(args.pretrained_contrast_encoder, map_location=device)
        contrast_encoder.load_state_dict(checkpoint_contrast_encoder['model_state_dict'])
    ...
    with torch.set_grad_enabled(False):
        for batch_id, image_dicts in enumerate(tqdm(valid_loader, desc='Evaluating')):
            # print(image_dicts)
            # save_images_from_dicts(image_dicts, "/iacl/pg23/prahlad/4imgs")
            # print(h)
            ...
            if batch_id % args.save_every == 0:
                ...
                # Create combined visualization with theta space and attention histograms
